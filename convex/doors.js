...
export const setOpen=mutation({
  ...
  handler:async(ctx,args)=>{
    ...
    if(!d)throw new Error('Porta desconhecida.');
    const saved=await ctx.db.query('doors').withIndex('by_room_door',q=>q.eq('room',args.room).eq('doorId',args.doorId)).unique();
    if((saved?.locked??d.locked)||!d.interactive)throw new Error('Porta trancada ou passagem fixa.');
    ...
    const body=p&&feet(p);
    if(!body||Math.hypot(Math.max(d.x-body.right,body.x-d.x-d.width,0),Math.max(d.y-body.bottom,body.y-d.y-d.height,0))>48)throw new Error('Aproxime-se da porta.');
    if(!args.open&&active.some(p=>{const b=feet(p);return b.x<d.x+d.width&&b.right>d.x&&b.y<d.y+d.height&&b.bottom>d.y;}))throw new Error('Passagem ocupada.');
