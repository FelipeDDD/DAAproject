...
export const send = mutation({
  ...
  handler: async(ctx,args)=>{
    ...
    if(!text||text.length>200)throw new Error('A mensagem deve ter entre 1 e 200 caracteres.');
    ...
    if(!player||player.sessionId!==args.sessionId||Date.now()-player.lastSeen>=15_000||
       player.room!==args.room||!['school','outside'].includes(args.room))throw new Error('Sessão ou sala inválida.');
