...
let identity,school=[],outside=[];
const unsub=[b.onUpdate(api.messages.inRoom,{room:'school'},rows=>school=rows),b.onUpdate(api.messages.inRoom,{room:'outside'},rows=>outside=rows)];
...
try{
  ...
  await a.mutation(api.players.update,{...identity,room:'school',x:800,y:750,direction:'down'});
  ...
  await a.mutation(api.players.update,{...identity,room:'outside',x:480,y:322,direction:'down'});
  ...
  console.log('PASS: realtime chat, selected author, room isolation and change of room.');
